...
const TICK = '\u2714'.green; // https://en.wikipedia.org/wiki/Check_mark
const CROSS = '\u2716'.red.bold; // https://en.wikipedia.org/wiki/X_mark
    ...
    async.waterfall([
        ...
        if (packages) {
            packages.forEach(function(p) {
                exports.dumpPackageSummary(p);
            });
        }

        if (err || exports.packagesInError(packages)) {
...
exports.loadConfig = function(file, cb) {
    ...
    try {
        ...
        exports.debug('Config file ' + file + ' loaded.');
        ...
        if (isDefaultConfigFile(fileNorm)) {
        ...
        if (err.code === 'MODULE_NOT_FOUND') {
            ...
            cb('Failed to parse ' + fileNorm, null);
            return;
        }
    }
...
var runCommands = function(packages, commandSet, cb) {
    ...
    async.waterfall([
        ...
        function(folders, done) {
            ...
            for (var f in folders) {
                ...
                //var packageJsonFile = isNodeProject(files[f]);
        ...
        function(packages, done) {
            ...
            exports.debug('Resolving local dependencies for ' + packages.length + ' Node modules.');
...
exports.packagesInError = function(packages) {
...
exports.dumpPackageSummary = function(p) {
    ...
    //var d = '  local dependencies: ' + (p.localDeps.length === 0 ? 'none' : p.localDeps.join(' '));
    //console.log(d.grey);
...
var runAll = function(packages, name, cmd, args, cb) {
...
function runShell(pkg, cmd, args, cb) {
    ...
    if (dryRun) {
        console.log('  [DryRun] Would chdir to ' + folder);
        console.log('  [DryRun] Would execute ' + args.join(' '));
        cb(null, null);
        return;
    }

    exports.debug('Chdir ' + folder);
    exports.debug('Execute ' + cmd + ' ' + args.join(' '));
...
function readPackage(f, cb) {
    ...
    if (!hasPackageJson) {
        // Ignore folders with no package.json file.
        cb(null, null);
        return;
    }
    ...
    var prjDeps = Object.keys(pkg.dependencies);
...
function getGitBranch(path, cb) {
    ...
    try {
        ...
        exports.debug('Checking for ' + path + '/.git: Found');
        ...
        // Not a git repo.
        exports.debug('Checking for ' + path + '/.git: Not Found');
        cb(null);
        return;
    }
}
